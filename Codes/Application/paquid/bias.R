...
MAB_beta <- sapply(1:6, function(i) mdae(matB[,i], fixef_est[,i]))
...
results_table <- data.frame(
  ...
  Bias = c(
    # Bias calculations
    abs(bias_E),
    abs(colMeans(fixef_est) - theta_orig$betas),
    abs(colMeans(D_est)-matrix(vech(theta_orig$D), nrow = 1, byrow = T)),
    abs(colMeans(shape_est) - theta_orig$shape),
    abs(colMeans(scale_est) - theta_orig$scale),
    abs(colMeans(omega_est) - theta_orig$omegas)
  ),
