...
pij_true <- list()
CE <- ibs_est_train <- ibs_est_test <-ISE_in <- ISE_out <- MSE_y_out <- numeric(B)
library(matrixcalc)
library(survival)
library(prodlim)
library(data.table)
...
# preds
  ...
  # }
  MSE_y_in <- Metrics::mse(actual = (result$data_long$y), predicted = (preds))
  
  CE[iter] <- mean(result$C  != true_components)
    data <- result$data_long
data_wide <- result$data_wide
result$theta_orig$betas <- t(theta_orig$betas)
true_comp <- estep(result$theta_orig$pj, result$theta_orig)$P
  # ISE_in[i]<- get_nnem_ISE(result, shape_est = shape_est[i,], scale_est=scale_est[i,], mix.prob = comp.p,result$theta_orig, true_comp)
...
result$theta_orig$betas <- t(theta_orig$betas)
true_comp <- estep(result$theta_orig$pj, result$theta_orig)$P
  # ISE_out[i] <- get_nnem_ISE(result, shape_est = shape_est[i,], scale_est=scale_est[i,], mix.prob = comp.p,result$theta_orig, true_comp)
      pred_y <- matrix(0, nrow(result$data_long), 2)
        Z <- model.matrix(~1, data = result$data_long)
    X <- model.matrix(~measure_time,data=result$data_long)
