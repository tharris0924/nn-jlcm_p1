...
nn_model <- nnet(
    x = x, # Input features
      ...
      maxit = 2000,    # Max iterations
      trace = FALSE,   # No output during training
      linout = TRUE    # Linear output for probabilities
    )
    
    # Predict probabilities
    prob1 <- predict(nn_model, as.matrix(cbind(data_wide$X1, data_wide$X2)))
...
init <- c(theta$betas, theta$cholesky[lower.tri(theta$cholesky, diag = T)],log(theta$sigma_e), theta$omegas) 
...
library(parallel)
n_cores <- detectCores() - 1
opt_result <- marqLevAlg::mla(init, fn=nll_marg,data=data, posterior=postprob,nproc=n_cores, print.info=T)
# opt_result <- obj
#  cl <- makeCluster(n_cores)     # set the number of processor cores
#  setDefaultCluster(cl=cl)
...
# obj <- opt_result
 (betas_matrix <- matrix(opt_result$b[1:6], nrow = 3, ncol=2,byrow = T))
  cholesky <- metafor::vec2mat(opt_result$b[7:9], diag=T)
  cholesky[1,2] <- 0
   (D <- tcrossprod(cholesky))
  (sd_e <- exp(opt_result$b[10]))
  (omegas <- opt_result$b[11:12])
...
opt_result <- tryCatch({
  marqLevAlg::mla(b=parms, fn=neg_loglik_wb, data_wide=data_wide, posterior=postprob)}, error = function(e) {
    optim(par=parms, fn=neg_loglik_wb, data_wide=data_wide, posterior=postprob)})



sd_res_Td <- summary(opt_result, loglik=T)
shapes <- exp(matrix(opt_result$b[1:m],ncol = 1)) # first m bameters are the shapes
scales <- exp(matrix(opt_result$b[(m+1):(2*m)])) # next m bameters are the scales
...
for(j in 1:m){
  # j <- 1
    ...
    D_g <- ((omegas_tmp[j])^2)*dgs  # random effect variance-covariance matrices
  ...
  bigs<-matrix(nrow = n, ncol = 2)
    for(i in 1:n){
      ...
      Xi_beta <- X_i %*% beta_g
      # xbs[i]<-list(Xi_beta)
      Z_i<-cbind(1, subset$measure_time)
      q<-ncol(Z_i)
      Y_i <- subset$y
      sigma<-sigma_g
      # calculating the distributional parameters for the group specific random effects      
      ...
      # expecting a 2by1 matrix for each group for each subject
      big <- D_g %*% t(Z_i) %*% matrix.inverse(Vi) %*% (Y_i - Xi_beta)
      bigs[i,] <- big
      }
  bi[j] <- list(bigs)
}
...
# update parameters
 theta <- list(pj=new_prior, scale=scales, shape=shapes, betas=betas_matrix, ranef=bi, D=dgs, cholesky=cholesky, sigma_e=sd_e, omegas=omegas)
