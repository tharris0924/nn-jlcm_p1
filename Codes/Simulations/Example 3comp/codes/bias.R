...
pij_true <- list()
CE <- ibs_est_train <- ibs_est_test <-ISE_in <- ISE_out<- numeric(B)
...
# library(dplyr)
  ...
  ISE_in[i] <- get_nnem_ISE(result, shape_est = shape_est[i,], scale_est=scale_est[i,], mix.prob = pij[[i]],theta_orig, pi_true)
  ...
  load(file)
  if(!is.null(result)){
  ISE_out[i] <- get_nnem_ISE(result, shape_est = shape_est[i,], scale_est=scale_est[i,], mix.prob = pij[[i]],theta_orig, pi_true)

  ibs_est_test[i]<- get_nnem_brier_score(shape_params=shape_est[i,], scale_params=scale_est[i,], mix.prob = pij[[i]], test_data=result$data_wide)
  }
  extra <- nchar('||100%')
  width <- options()$width
  iter <- i
...
ibs_est_test_est <- mean(ibs_est_test[idxs_to_keep], na.rm = T)
ibs_est_train_est<- mean(ibs_est_train[idxs_to_keep], na.rm = T)
ise_in <- mean(ISE_in, na.rm = T)
ise_out <- mean(ISE_out, na.rm = T)
nnem_ce <- mean(CE)
B <- nrow(scale_est)
