...
mu <- c()
  for (i in 1:n) {
    ...
    R_blocks[[i]] <- (sigma_e^2) * diag(length(idx))
    V_blocks[[i]] <- V_i
    mu <- c(mu, XBg)
  }
...
tcrossprod(cholesky)
omegas <- alphas[1:2]
theta_orig <- theta <- list(pj=pi_init, scale=scales, shape=shapes, betas=betas_matrix, ranef=bi, 
  D=D_g_og, cholesky=cholesky, sigma_e=sd_e, omegas=omegas)

# save(theta_orig, file="theta_orig_k3.RData")
