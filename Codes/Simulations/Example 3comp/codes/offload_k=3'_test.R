...
for(n in sample_sizes[1]) {
  ...
  for(c_rate in censoring_rates[1]) {
    ...
    # Parallel processing for NNEM only
    offload <- foreach(
      iter = 1:1000,
      .packages = c("matrixcalc", "prodlim", "data.table", "survival", "SurvMetrics"),
      .export =ls(),
      .combine = c, 
      .verbose = TRUE, .errorhandling = "pass"
    ) %dopar% {
      # iter <- 1
      # Load NNEM results from k=3 directory
      ...
      if(file.exists(file_nnem)) {
        ...
        if(!is.null(theta)){
          ...
          # True mixing proportions for k=3 (from your bias.R)
          ...
          # Assuming you have theta_orig available for ISE calculation
          ...
          # Longitudinal prediction (similar to k=2 but for 3 components)
          ...
          if(file.exists(file_test)) {
            ...
            if(!is.null(result)){
              ...
              # Use training parameters with test data
              pred_y <- matrix(0, nrow(result$data_long), 3)  # 3 components
              Z <- model.matrix(~measure_time, data = result$data_long)
              X <- model.matrix(~measure_time, data=result$data_long)
              ...
              # Get test data predictions using training model
              # test_comp.p <- estep(prior_mixing = theta$pj, theta)$P
    ...
    for(result in offload) {
      if(!is.null(result) && !is.null(result$iter)) {
        iter <- result$iter
