...
nll_marg <- function(parms, data, posterior, Z){
...
marginal_density_yis <- function(data_long, betas_matrix, cholesky, sigmas) {
  ...
  init <- parms
  betas_matrix <- matrix(init[1:12], nrow = 4, ncol=3)
  cholesky <- metafor::vec2mat(init[13:18], diag=T)
...
nll_marg(init, data, postprob, Z=Z)
library(parallel)
n_cores <- detectCores() - 1
opt_result <- marqLevAlg::mla(init, fn=nll_marg,data=data, posterior=postprob,nproc=n_cores, print.info=T, Z=Z)
# opt_result <- obj
#  cl <- makeCluster(n_cores)     # set the number of processor cores
#  setDefaultCluster(cl=cl)
...
# obj <- opt_result
(betas_matrix <- matrix(opt_result$b[1:12], nrow = 4, ncol=3))
  cholesky <- metafor::vec2mat(opt_result$b[13:18], diag=T)
  cholesky[1,2] <- cholesky[1,3] <- cholesky[2,3]<- 0
   (D <- tcrossprod(cholesky))
  (sd_e <- exp(opt_result$b[19]))
...
# update parameters
 theta <- list(pj=new_prior, scale=scales, shape=shapes, betas=betas_matrix, ranef=bi, D=dgs, cholesky=cholesky, sigma_e=sd_e)
